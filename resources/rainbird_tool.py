...
```
$ ./rainbird_tool --help
$ ./rainbird_tool get_wifi_settings host password
```
"""
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        client = async_client.CreateController(session, args.host, args.password)
        method_args = {
            k: parse_value(v)
            for k, v in vars(args).items()
            if k != "func" and k != "command" and k != "log_level" and k != "host" and k != "password"
        }
